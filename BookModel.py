...
class Book(db.Model):
    ...
    def get_all_book():
        return [Book.json_data(book) for book in Book.query.all()]

    def get_book(_isbn):
        return Book.json_data(Book.query.filter_by(isbn=_isbn).first())

    def delete_book(_isbn):
        is_sucessfull = Book.query.filter_by(isbn=_isbn).delete()
        db.session.commit()
        return bool(is_sucessfull)
    ...
    def update_book(_isbn, _name, _price):
